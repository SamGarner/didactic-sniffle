...
class Game
  attr_accessor :board, :codebreaker
...
class Board
  attr_accessor :guess_counter, :board_array
...
class Player
  ...
  def count_exact_matches
    @turn_check = @board.board_array[0]
    (0..3).each do |n|
      if @new_guess_array[n] == @board.board_array[0][n]
        @correct_count += 1
